...
def superuser_and_reauthed(view):
    ...
    Platform-operator actions (tenant CRUD, seeding admins) live behind this:
    they reach past one tenant, so they must never be reachable by a per-tenant
    admin — the superuser flag is the only cross-tenant guard."""
    @wraps(view)
    def inner(request, *args, **kwargs):
        if not reauth_ok(request):
            return JsonResponse({'status': 'reauth_required'}, status=403)
