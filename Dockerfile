...
# its own libpq, and nothing is compiled at runtime. Backups are per-tenant dumps
# written by the app itself and uploaded over SFTP with paramiko (pure Python), so
# no postgres client, rsync or ssh binaries are needed.
RUN apt-get update && apt-get install -y --no-install-recommends \
