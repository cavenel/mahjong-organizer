# Admin console — head scorer guide (MCR)
...
This guide is for a **head scorer** on an **MCR** (Mahjong Competition Rules)
tournament — someone holding all three non-staff roles: **Scorer**, **Publisher**
and **Display operator**. It is written to be **read in order**, following the
timeline of an event: sign in, enter scores, publish rounds, drive the screens,
run the ceremony.

> **This is the MCR head-scorer edition** (scorer + publisher + display operator).
> A scorer who only enters scores can use the shorter
> [scorer guide (MCR)](MCR_scorers.md). Staff tools (importing/preparing the
> tournament, creating users, EMA export) and the complete reference live in
...
single-role [scorer guide](MCR_scorers.md)).
...
### MCR scoring (Table Points & score sheets)

This is an **MCR** tournament, so scoring uses **Table Points (TP)** derived from
**Minipoints (MP)**, and each table has a **per-hand score sheet** (16 hands) that
can be filled by hand or by **scanning** a paper sheet. (On a Riichi tournament
only Minipoints are entered and there's no score sheet — see
[Riichi_head_scorer.md](Riichi_head_scorer.md).)
...
> 🔑 **Who can do this:** *Scorers.* A **scorer** enters and corrects table
> results, can open a per-table **score sheet** to enter every hand, and can
...
| **Sheets in progress** | Tables whose per-table score sheet has been started but not yet validated. |
| **Sheets validated** | Tables whose score sheet has been validated. |
...
| Rehearse with fake data | `test.mahj.ovh` → Scoring → **Fill all rounds** | (see §10) |
...
| Button | What it does |
|---|---|
| **Fill all rounds — scores** | Fills the four seats of every table in **every round** with random Minipoints that sum to zero, auto-computes Table Points, and saves. If you're a publisher it then **publishes rounds in order** too. |
