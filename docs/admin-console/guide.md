...
| **Head scorer** (scorer + publisher + display operator) | Parts I to IV. Printable one-page recaps: [scorer cheat sheet](mcr-scorer-cheat-sheet.md) and [head-scorer cheat sheet](mcr-head-scorer-cheat-sheet.md) (MCR) |
...
**MCR vs Riichi.** The console adapts to the tournament's rule set, which is
chosen in the tournament settings.
...
- **Riichi** and others: only **Minipoints** are entered. There is no TP column,
  no score sheet and no scan tool. Sections that only apply to MCR are marked
...
- **Admin** → the *Dashboard*.
- **Scorer** → the *Scoring* page. Publishers land there too, because the
  publish toggles live there.
...
sent to the login page. Enter your username and password. You can also use a
**login link** an admin sent you, which signs you in with no password at all.
You then land on your role's default page.
...
The console is split into **two workspaces**, one per phase of a tournament:

- **Setup** is everything you do *before* play: settings, players, seating, the
  draw, printing, accounts. Sky-blue accent.
- **Run** is everything you do *during* play: scoring, publishing, the screens,
  the ceremony, the exports. Amber accent.

Tournament admins switch between them with the **Setup / Run** toggle at the top
of the sidebar. Scorers, publishers and display operators only have Run, so the
toggle is not shown to them. Once a round has been scored or published (or the
...
**Setup** is for tournament admins only. It opens on the **Setup checklist**,
then these groups:
...
| Administration | User management · Backup & restore · Publish target · Scanning *(when scanning is switched on)* |

**Run** is everyone's side. It opens on the **Dashboard**, which every role sees.
...
(Platform operators see one more entry under *Administration*, **Tenants**. See
[Part VI](#part-vi-platform-operator).)
...
*Print* sends the page to the browser's print dialog. Use "Save as PDF" there to
export.

- **Setup → Print materials** *(admins)*: one card per printout. It covers
  player names, team names *(team events)*, player cards, table positions, cross
  positions (also by team), and the schedule, which is the paper the room needs
  before play. The page tells you whether the draw is complete, since cards and
  positions depend on draw numbers.
- **Setup → Player card design** *(admins)*: how the player cards look. See
...
look. Everything saves as you edit. The preview beside the controls is the real
print page, so what you see is what comes out of the printer.

- **Card format.** **A6 portrait** (4 per A4 sheet) is the full-size card with a
  tall header. **A7 landscape** (8 per sheet) is half that, for short
  tournaments: a compact one-line header, and the period, session count and
  ruleset along the bottom edge where the A6 card carries them in its header.
  It holds eight rounds over one day, seven over two and six over three (each
  day adds a heading), and the page warns you above eight. The preview is the
  real check either way, because rows that do not fit run off the bottom of the
  card there. Print **double-sided, flipping on the long edge**. The back of
  each sheet is laid out for that, so every card gets its own opponents on the
  back.

  Both formats keep a 7&nbsp;mm border clear on all four edges of the sheet,
  since few printers can print right to the paper edge. A card therefore comes
  out a little under a true A6 or A7, at 98&nbsp;×&nbsp;141.5&nbsp;mm and
  98&nbsp;×&nbsp;70.75&nbsp;mm.
- **Theme.** *Classic* uses colour bars and filled seat chips. *Minimal* is ink
  on paper, with no colour bars. *Bold* has a filled header band that reads
  across a room. Switching theme replaces the colours below. If you have edited
  the CSS yourself, you are asked first.
- **Colours.** Pick a preset palette, or set any colour individually. The
  ...
  actually uses.
- **Advanced, edit the CSS.** This is the whole card stylesheet, for when you
  want more than colours: hide a field, change a font, restyle the seat chips.
  Your CSS is added *after* the built-in one, so any rule you write wins, and
  the panel lists the class names to target. Cards may not load anything from
  another site, so `@import` and remote `url()` are rejected with a message.
...
The console and screens stay in sync over WebSockets, so you rarely need to
refresh:
...
- **The public site** refreshes only when a round is **published/unpublished**,
  so the crowd never sees scores before they are official.

If a page ever looks stuck, refresh that browser tab. It rejoins at the current
state.
...
- The **Sum** cell turns **green (`0`)** if the four MP add up to zero (a valid
  table). It turns **red** and shows the non-zero total otherwise.
...
> `-20`). **Leave it as it is.** The MP you enter, with the penalty already
> applied, are the official scores. Record each penalty on the **score sheet**
> (the *Penalties* row) so its after-penalty Total and Table Points match what
> you entered. The score sheet's raw hand total will then differ from the
> entered MP/TP by exactly the penalties, which is normal. Penalties recorded on
> the score sheet also show on the public per-table detail view.

Scores **save automatically** about 2 seconds after you stop typing, and
immediately if you navigate away. There is no "Save" button.
...
overwritten for a few seconds, so you will not lose your in-progress typing. If
two scorers do save conflicting numbers for the same table, the save that
arrives second is refused. That scorer's row snaps back to what was actually
...
- **Tab** runs filter → first seat → … → last seat and then loops back to the
  filter, skipping rows hidden by the filter. You can enter table after table
...
**running totals**. It shows the derived **per-player total (MP)** and **Table
...
This cross-check catches data-entry mistakes. A correctly entered score sheet
produces green totals that match the grid.

### Penalties

Just above the **Total** row the sheet has a **Penalties** row, with one integer
box per player. Enter a penalty as a whole number, positive or negative (e.g.
...
The sheet's totals then line up with the penalised MP/TP entered on the grid.
...
a validated sheet cannot be edited by accident. Untick to edit again.
...
> Score sheets save per-hand automatically, about 1 second after each edit,
> using an optimistic version lock. If two people edit the same hand at once,
> the second save is rejected, so no edit is silently lost.
...
confirming it, clears the tint. You can then quickly eyeball which scanned
...
> - The page handles being offline or a lapsed session gracefully and tells you
...
*Deciding when each round becomes official, which means visible on the public
website and the leaderboard screens. Publishing is a separate role from scoring:
...
- **Publish round N** is the checkbox that publishes and unpublishes the round.
- **Status** shows *Published* (green) or *Not published*, on the right.
...
Tick **Publish round N**. The console enforces these rules, and the same checks
run on the server, so they cannot be bypassed:
...
2. **Rounds publish in order.** You cannot publish round N until rounds 1…N-1
   are already published. No gaps are allowed.

On success:

- The round's scores become **locked**. Score inputs in that round's grid turn
  grey and read-only, and scorers can no longer edit them. Publishing freezes
...
it**. If rounds 1 to 5 are published and you unpublish round 3, rounds 3, 4 and
5 all become unpublished and editable again. Re-publish when corrected.
...
public. It publishes the round with the result **hidden**, which preserves
...
2. **Display operator:** run the **Ceremony console**
   (see [§13](#13-the-prize-giving-ceremony)) to reveal teams and players place
   by place, and finally press **Publish to everyone & end**. That button is
   what makes the complete final results public.
...
whole tournament, **one row per round**. You can see how far each round has
progressed and publish without hunting through the round tabs.
...
*Everything the audience sees on the room's projectors and TV screens: which
view each screen shows, the round timer with its synchronized start gong, the
...
A **screen** record in the app maps each URL to a **view**, which is what to
show. Add as many screen records as you have physical displays, point each
display's browser at the matching `/<n>` URL, and then change views centrally
from the console. Every screen reacts live.
...
- **Add screen** appends a new screen, starting on *Blank*. **Remove screen**
...
- Each **screen card** is headed `/N — name`. Click the name to rename the
  screen, for example to "Main hall left", so cards are identifiable at a
  glance.
- **Identify screens** flashes each screen's number in its corner for a few
  seconds, so you can match physical projectors to their URLs without walking
  over.
...
appear as amber tiles. Click a tile to apply it, click its **✕** to delete it.
...
screen. You can then confirm what is on each projector without walking over to
it. Click a thumbnail's **enlarge** icon for full size. Previews only load
while shown, to save bandwidth.
...
- **Start timer** starts the round. There is a brief lead-in and a **3-2-1
...
- While running, the panel shows the **time remaining** and a *Running* badge,
  and the button becomes **Reset timer**. When time is up the badge reads *Done*.
- Resetting a running timer asks for confirmation.

The timer is **server-authoritative**. The server owns the official start
instant and every screen renders from it, so screens cannot drift apart or be
reset by a stray reload. The round length is the **Total time of a round**
tournament setting, which a tournament admin sets (see
...
reveal sequence for the awards. The **public website stays in suspense** until
...
- **On screens now** is a live status line telling you what the audience
  currently sees.
- **Screen previews** are the same live previews as the Display page, so you can
...
- **Start** puts that section on the screens, empty and ready.
- **Reveal next ▸** reveals the next place, lowest rank first, working up to
  1st. Press once per place.
- **◂ Back** undoes the last reveal.
...
- **Teams** shows only the **top 3**, the prize-winners, each with team name,
  all member names, and the team total (TP / MP). Individual player scores are
  not shown.
...
console itself can be reloaded too, and it resumes where you left off.
...
**Setup checklist** is the Setup home, and the admin's landing page until play
starts. It tracks readiness: players listed, seating chart in place (and sized
for the list), draw complete, screens configured (optional). Each step links to
the page that completes it, and to **Print materials**. When the required steps
are all done it offers **Go to Run →**. A failed template import also reports
here.

> ![Setup checklist](screenshots/06-setup-checklist.png)<br>
> 📸 **Screenshot, the Setup checklist.**

**Dashboard** is the Run home, and the landing page once a round has been scored
or published. It shows **Rounds** progress (scored / published) and the **round
timer** live. When web publishing is configured it also shows the **Publish to
web** button.
...
- **Identity.** Title (short, for screens and browser tabs), full tournament
  name (report headers and printouts), city, period, and two optional fields:
  the **home nation**, which enables a national sub-ranking, and the
  **federation code**, which is stamped on the EMA report.
- **Format.** The **ruleset** (MCR or Riichi), which shows or hides the TP
  column, score sheets and scan tool, and the **teams** toggle, which controls
  team standings, columns and printouts.
- **Rounds.** The **number of rounds** and the **total time of a round**, which
  is the timer's countdown length in seconds (e.g. `6900` = 1 h 55 m).
- **Schedule.** The agenda table, with day, time and name per row, and a
  *Round* checkbox marking actual playing rounds. Mark exactly *number of
  rounds* rows as rounds so per-round times line up.
- **Tournament logo.** A PNG (max 2 MB) shown on screens and printouts in place
  of the default.
- **Danger zone, Reset tournament.** This permanently deletes *everything* for
  this tournament (players, seating, scores, published rounds, schedule) and
...
session cannot reach it. It shows only **this tournament's** users.

- **Add a user.** Enter a username plus their roles. Roles are the checkboxes on
  each row and can be changed at any time: **Admin**, **Scorer**, **Display
  operator**, **Publisher** (see [Roles at a glance](#roles-at-a-glance)). A
  publisher can also score, so there is no need to tick both.
- **Login links.** Generate a **passwordless login link** for a user and hand it
  out. A link is valid for a limited number of days and can be invalidated.
  *Invalidate login links* revokes all of a user's outstanding links. This
  suits volunteers and kiosk laptops, since there is no password to type or
  forget.
- **Remove** takes a user's access to *this* tournament away. The console
  refuses to remove or demote the **last admin**, so a tournament cannot lock
  itself out.
- A user marked **shared** also belongs to another tournament on this server.
  Their credentials are then managed by the platform operator, and adding an
  existing account to a new tournament is a platform-operator action.

**Checking what a role sees.** The avatar menu at the bottom of the sidebar has
a **Preview as** section for admins. Pick *Scorer*, *Publisher* or *Display
operator* and the whole console (sidebar, pages, permissions) becomes what that
account gets, with a banner on every page and **Back to admin view** to return.
Use it before handing out accounts, and for following this guide's role parts.
...
1. **Tournament settings.** Set the title, ruleset, teams toggle, **number of
   rounds** and the schedule ([§15](#15-tournament-settings)).
2. **Edit players.** Click **Edit players** to unlock the table, then
   **Add player** (below the table) once per competitor. Each new row arrives
   named *Player n*, where n is its position in the list, with the cursor on its
   name. Type the real name, country, EMA id and team over it. Each change saves
   as you leave the field. The **×** at the end of a row removes a player.
   **Done editing** warns if the total is not a multiple of 4 (seating needs
   full tables) or if rows are still named *Player n*.
3. **Seating → Generate** (below).

**B. From the Excel template:**

1. **Excel import / export.** Download the blank Excel template, fill it in, and
   upload it. You can also **export the current tournament** in the same format
   to back it up or edit it offline. The upload creates the players, the seating
   chart for every round, the schedule, and the tournament options in one go.
   - ⚠️ Importing **replaces the whole tournament** with what the file holds.
     The console warns you, naming what will be replaced.
   - A wrong or old-format file is **rejected with the tournament untouched**.
     That covers missing sheets, an unreadable rounds count, a player count that
     is not a multiple of 4, duplicate draw numbers, an unreadable score cell,
     and so on.
   - **Export current tournament** always writes the setup: settings, players,
     ...
     seat recorded, so MP, penalty, table points under MCR, and how far its
     sheet got), *Score sheets* (every played hand: winner, discarder, value)
     and *Standings* (the ranked table). Which rounds are published travels with
     them.
   - **The import reads all of that back**, so a workbook restores the
     ...
     sheets. The *Standings* tab is derived, so edits there are ignored, and the
     tab says so in its first row.
   - Columns are matched by their header rather than their position, and winds
     may be written `E`/`S`/`W`/`N`, `East`…`North` or `1` to `4`, so a workbook
...
2. **Seating.** Inspect the current seating chart's quality, meaning how well
   opponents are spread. You can also **generate a seating in the app** instead
   of importing one: preview a chart, compare quality measures, then apply.
   Applying a new seating clears any entered scores.
3. **Edit players.** Add or remove players, correct names, EMA ids, countries
   and teams in place, and assign **draw numbers**, which set the seat-slot each
   player occupies, once seats exist. Add and remove players **before** the
   seating is generated. The chart is built for the list's size, and if the two
   later differ the Dashboard and the Seating page show a red warning until you
   regenerate the seating or fix the list. A player who withdraws mid-tournament
   is *not* removed, because their seat has to stay in the chart. Rename the row
   (for example to the substitute) or leave it as is.
4. **Randomize players (live)** and **Team draw (live)** are full-screen draw
   pages to run the draw *as a show* in front of the players. Each opens in its
   own tab.
5. **Print materials** gives you the paper for the room: player names, team
   names, player cards, table positions, cross positions (also by team), and the
...
**Setup → Administration → Publish target** configures **static web publishing**.
On every round publish, the public site is rendered to static files and uploaded
by SFTP to a plain web host. Use it when spectators should follow on a
separate public website. Configure host, port, user, remote path and a password
*or* private key. **Test connection** verifies the setup. The same page sets the
...
backup in a browser. The trade is that anyone who finds the address can read a
backup, and a backup holds every entered score, including rounds you have not
published yet and the ceremony's reveal state. It carries no passwords or API
keys.

## 19. Backing up and restoring a tournament

**Setup → Administration → Backup & restore** deals in **dump files**. One file
holds the entire tournament: settings, the player list, the seating, every
entered score, which rounds are published, the schedule, the screens and the
round timer.
...
- **Download dump** saves one now. If web publishing is configured, one is also
  ...
  published site, where you can browse and download them. That link appears once
  the **Spectator URL** is set on *Publish target*. Without a publish target,
  downloading is the only copy, so do it after each round.
...
app version. That is how a tournament moves onto a venue laptop, or back off it,
when the server is unreachable. If the file was made for a different tenant, the
confirm dialog says so before you commit.

> ![Backup & restore](screenshots/41-backup-restore.png)<br>
> 📸 **Screenshot, Backup & restore page.**


## 20. After the tournament, the EMA export
...
*The superuser who runs the server, who is not part of the tournament crew.
Their Setup sidebar shows one extra entry under Administration:*

- **Tenants** creates and manages tournaments, each on its own subdomain, and
  jumps into any tenant's user management.

The raw Django admin (`/admin_db/`, superuser only) is **not linked** from the
console. Everything a tournament needs is editable from the pages above, and the
raw admin lists every tournament's data unscoped. It is a rescue tool, so type
the URL when you need it.

Setting up the server itself, meaning Docker, DNS/TLS, environment variables,
backups and the standalone venue-laptop build, is documented in
...
**without touching a real event**. Use it to train scorers, check how the
leaderboard and screens look with data, and practise the prize-giving ceremony.
Any tenant, including a standalone install, becomes one by ticking **This is a
test tournament** on **Tournament settings**. On a multi-tenant host the
convention is a dedicated tenant such as:
...
There are two functional differences from a real tournament: a **fake-data
...
production, so the rehearsal is faithful.
...
is on, a team, four to a team. They are not drawn in, so use the normal draw
flow. Importing or re-generating the seating wipes any previous scores on the
tenant, which is what you want here.
...
| Button | What it does |
|---|---|
| **Fill all rounds — scores** | Fills every table in **every round** with random Minipoints that sum to zero, auto-computes Table Points, and saves. If you are a publisher/admin it then **publishes rounds in order** too. |
...
| **This round only: Fill / Clear …** | The same four actions scoped to the **open round tab** and publishing nothing. Use it to step a tournament forward one round at a time and watch the badges, publish gates and displays react. |
...
totals on the Scoring grid, which is what the leaderboard uses. *Score sheets*
are the detailed 16 hands behind each table (MCR only). For a quick
leaderboard or ceremony rehearsal you usually only need **Fill all rounds -
scores**.
...
3. **Publishing.** As a publisher ([§9](#9-publishing-and-unpublishing)):
   publish and unpublish rounds, and see the lock behaviour and the cascade on
   unpublish.
4. **Ceremony.** As a display operator: run the **Ceremony console**
   ([§13](#13-the-prize-giving-ceremony)) end to end. The incomplete last round
   lets you practise the suspense and reveal flow.
5. **Scanning.** Open a score sheet and try the **QR → scan** path on a phone.
   This requires the OCR service to be configured on the host.

**Reset between rehearsals.** *Clear all rounds* wipes results but keeps the
players and seating. Re-running the **Excel import / export** upload resets the
whole tenant.
...
- Random fills produce **valid** tables, where each table's Minipoints sum to
  zero, so the leaderboard maths and the score-sheet cross-checks behave like
  the real thing.
- You still need the appropriate **role** for each action on a test tournament.
  That also makes it a good place to confirm a new user's roles before the real
  event.
