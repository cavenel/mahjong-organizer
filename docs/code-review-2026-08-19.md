...
- ✅ DONE (post-S5) — **Structural** — `options()` was a ~480-line page dispatcher with 13 hand-rolled role gates; it now dispatches through an `ADMIN_PAGES` table, pinned by a role × page matrix test. The tunneled mutations keep their URL: S2's 405 guards had already closed F6, so moving them would have changed every posting template for no security gain.
